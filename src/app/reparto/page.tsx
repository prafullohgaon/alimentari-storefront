...
import { SlidersHorizontal, ArrowUpDown, Check, X, ChevronRight } from "lucide-react";
...
function RepartoContent() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-8 flex flex-col font-sans">
      ...
      {/* Main Grid */}
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 md:px-6 py-6 md:py-8 space-y-8">
        
        {/* Department Header */}
        <div className="flex items-center justify-between border-b border-border/80 pb-5 select-none">
        ...
        {/* Double-Column Browser Wrapper */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          
          {/* Left Column: Sidebar Filters (Hidden on mobile) */}
          <aside className="hidden lg:block space-y-6 sticky top-28 self-start h-[calc(100vh-140px)] overflow-y-auto pr-2 scrollbar-none select-none">
            {/* Header / Clear all */}
            <div className="flex justify-between items-center border-b border-border pb-3">
              <h3 className="font-serif text-lg font-bold flex items-center gap-2">
                <SlidersHorizontal className="w-4 h-4 text-primary" /> Filtra Spesa
              </h3>
              <button
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex flex-col gap-1.5 font-semibold text-sm">
                <button
                  ...
                  className={cn(
                    "text-left px-3 py-2 rounded-lg transition-colors flex justify-between items-center",
                    selectedDept === "tutti" ? "bg-primary text-primary-foreground" : "hover:bg-muted/10 text-muted-foreground hover:text-foreground"
                  )}
                >
                ...
                {CATEGORIES.map((cat) => (
                  <button
                    ...
                    className={cn(
                      "text-left px-3 py-2 rounded-lg transition-colors flex justify-between items-center capitalize",
                      selectedDept === cat.id ? "bg-primary text-primary-foreground" : "hover:bg-muted/10 text-muted-foreground hover:text-foreground"
                    )}
                  >
                    <span>{cat.name}</span>
            ...
            <div className="space-y-2.5 pt-4 border-t border-border/60">
              ...
              <div className="space-y-2 px-1">
                ...
                <div className="flex justify-between text-xs font-semibold text-foreground">
                  ...
                  <span className="text-primary font-bold">Fino a €{localMaxPrice.toFixed(2)}</span>
                </div>
              </div>
            </div>

            {/* Organic Switch */}
            <div className="pt-4 border-t border-border/60 flex items-center justify-between px-1">
              ...
              <button
                ...
                className={cn(
                  ...
                  onlyOrganic ? "bg-success" : "bg-border"
                )}
              >
                <span
                  className={cn(
                    "pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out",
                    onlyOrganic ? "translate-x-5" : "translate-x-0"
                  )}
                />
              </button>
            </div>
                ...
                {DIETARY_OPTIONS.map((diet) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-2.5 text-xs font-semibold text-muted-foreground hover:text-foreground transition-colors w-full text-left"
                    >
                      <div className={cn("w-4 h-4 rounded border flex items-center justify-center transition-colors", isChecked ? "bg-primary border-primary text-white" : "border-border bg-card")}>
            ...
            {/* Brands Checklist */}
            <div className="space-y-3 pt-4 border-t border-border/60">
              <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest pl-1">Marchi Selezionati</h4>
              <div className="space-y-2 px-1">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {/* Sorting & Mobile Filter Triggers top row */}
            <div className="flex justify-between items-center bg-card border border-border/80 rounded-xl px-4 py-3 shadow-soft select-none">
              {/* Mobile trigger filter */}
              <button
                onClick={() => setMobileFilterOpen(true)}
                className="lg:hidden h-10 px-4 border border-border bg-transparent text-foreground hover:bg-muted/10 rounded-lg font-bold text-xs flex items-center gap-2 active:scale-95 transition-all btn-touch-active"
              >
                <SlidersHorizontal className="w-4 h-4 text-primary" /> Filtra e Ordina
              </button>
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <select
                  value={sortOption}
                  onChange={(e) => { updateFiltersInUrl({ sort: e.target.value }); }}
                  className="bg-transparent text-xs font-bold text-foreground outline-none border-none cursor-pointer pr-4"
                >
                  <option value="default">Rilevanza</option>
                  <option value="price-asc">Prezzo: Crescente</option>
                  <option value="price-desc">Prezzo: Decrescente</option>
                  <option value="rating">Top Recensioni</option>
                </select>
              </div>
            </div>
            ...
            {totalPages > 1 && (
              <div className="flex justify-between items-center pt-8 border-t border-border/60 select-none">
                ...
                <span className="text-xs font-semibold text-muted-foreground">
                  Pagina <strong className="text-foreground">{currentPage}</strong> di {totalPages}
                </span>
                <Button
                  variant="outline"
                  size="md"
          ...
          <div className="fixed bottom-0 left-0 right-0 max-h-[85vh] bg-card border-t border-border rounded-t-2xl shadow-elevation flex flex-col z-10">
            <div className="px-4 py-4 border-b border-border flex justify-between items-center">
              ...
              <button
                ...
                className="w-11 h-11 flex items-center justify-center rounded-full hover:bg-muted/15 btn-touch-active"
